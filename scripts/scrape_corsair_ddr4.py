#!/usr/bin/env python3
"""Scrape verified Corsair DDR4 desktop memory SKUs from corsair.com."""
...
OUT = ROOT / "data" / "corsair_ddr4.json"
...
def parse_profiles(raw: str | None, sku: str) -> list[str]:
    if not raw:
        ...
        if m and m.group("profile").upper() in {"C", "D"}:
            return ["xmp"]
        return ["jedec"]
    text = raw.lower()
    if "xmp" in text:
...
def parse_memory_size(raw: str | None, sku: str) -> tuple[int, int] | None:
    ...
    cap_match = re.search(r"CM[A-Z](\d+)GX4", sku.upper())
    if not cap_match:
        return None
...
def is_desktop_ddr4(item: dict) -> bool:
    sku = item.get("sku", "")
    if not PART_RE.match(sku.upper()):
        return False
    specs = spec_map(item)
    mem_type = specs.get("Memory Type", "DDR4").upper()
    if "DDR4" not in mem_type:
        return False
...
def item_to_entry(item: dict) -> dict | None:
    ...
    if specs.get("Package Memory Format", "UDIMM").upper() != "UDIMM":
        return None
    if "DDR4" not in specs.get("Memory Type", "DDR4").upper():
        return None
...
def sitemap_skus() -> dict[str, str]:
    ...
    for loc in root.findall(".//sm:loc", ns):
        ...
        if "/us/en/p/memory/" not in url:
            continue
        slug = url.lower()
        if "sodimm" in slug or "cmsx" in slug:
            continue
        m = re.search(r"/p/memory/([a-z0-9]+)/", slug)
        if not m:
            continue
        sku = m.group(1).upper()
        if "GX4" not in sku or not sku.startswith("CM"):
            continue
        out[sku] = url.rstrip("/")
    return out
